        ...
        <AroundTragetBorder>
            <div class="pic">
                <Transition name="uianim">
                    <img v-if="playerStore.song.pic" :src="playerStore.song.pic" alt="">
                ...
                <div class="singleLineTextEl title">{{ playerStore.song.title }}</div>
            <div class="singleLineTextEl singer">{{ playerStore.song.singer }}</div>
            <div @dblclick="openOriginLink(parseOriginLink(playerStore.song))" class="singleLineTextEl type">{{ playerStore.song.type }}</div>
            <div class="playProgressTip">
                <div class="cur">{{ playerStore.config.curTime }}</div>
                <div class="total">{{ playerStore.config.durationTime }}</div>
            </div>
            <div ref="playProgress" @click="changePlayProgress" class="playProgress">
                <div class="fill" :style="{width: `${playerStore.config.progress}%`}"></div>
            </div>
            <div class="volumeProgressTip">
                <div class="tip">Volume</div>
                <div class="total">{{ Math.round(playerStore.config.volume * 100) }}</div>
            </div>
            <div ref="volumeProgress" @click="changeVolumeProgress" class="volumeProgress">
                <div class="fill" :style="{width: `${playerStore.config.volume * 100}%`}"></div>
            </div>
            <div class="controlButtons">
              ...
              <VDropdown placement="right" popperClass="langPopperContainer">
                <div
                    v-show="Object.keys(playerStore.song.lrcs).length !== 0"
                    class="translate">
...
function parseOriginLink(song: songInPlay) {
    ...
    }else if (song.origin.type === 'netease') {
...
function changePlayProgress(e: any) {
    if (playProgress.value) {
        emitter.emit('changeCurTimeTo',minmax(playerStore.config.duration * e.layerX / playProgress.value.clientWidth, 0, playerStore.config.duration));
    }
}
function changeVolumeProgress(e: any) {
    if (volumeProgress.value) {
        playerStore.config.volume = minmax(e.layerX / volumeProgress.value.clientWidth, 0, 1)
    }
}
